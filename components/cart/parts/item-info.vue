    ...
    <div class="product-info info1">
        ...
        <div class="product-item">
            <div class="item-list-container horizon">
                <div class="item-list">
                    <!-- 체크박스 -->
                    <label v-if="disabled == false" class="circle-checkbox">
                        <input type="checkbox" class="checkbox sub" :class="disabled != false ? 'check-sold-out' : ''"
                               v-model="selectedItem" :value="buyItem.cartId" v-if="!buyItem.itemSoldOut"><i></i>
                    </label>
...
export default {
    ...
    methods: {
        ...
        freeGiftItemPopupOpen(buyItem) {
            vm.$emit('freeGiftItemPopupOpen', 'cart-list', buyItem.cartId);
        },
    }
}
</script>

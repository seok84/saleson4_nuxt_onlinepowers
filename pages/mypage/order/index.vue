<template>
    <section class="mypage-order">
                    ...
                    <div class="order-item">
                        ...
                        <div class="order-status">
                            <template v-for="(data, index) in o.items">
                                <OrderItem :order-code="o.orderCode" :item="data" @orderPopup="orderPopup" @deliveryLink="deliveryLink" />
                            </template>
                        </div>
                    </div>
                </template>
            ...
            <ValidationObserver ref="observer" v-slot="{ validate }">
                <form @submit.prevent="returnProcess(validate)">
                    ...
                    <ValidationProvider tag="div" rules="required" v-slot="{ errors }">
                        <div class="form-line" v-show="popup.returnInfo.param.claimReasonTextShow">
        ...
        <modal-default :class="{ 'show': modal.exchangeShow }" :title="'교환신청'" :bodyClass="'p-2 type-exchange'"  @modalClose="modal.exchangeShow = false" class="modal-product">
            <ValidationObserver ref="observer" v-slot="{ validate }">
                <form @submit.prevent="exchangeProcess(validate)">
                    ...
                    <ValidationProvider tag="div" rules="required" v-slot="{ errors }">
                        <div class="form-line" v-show="popup.cancelInfo.param.claimReasonTextShow">
                    ...
                    <ValidationProvider tag="div" rules="required" v-slot="{ errors }" v-if="popup.exchangeInfo.param.exchangeShippingAskType == '2'">
                        <div class="form-line package" v-show="popup.exchangeInfo.param.exchangeShippingAskType != '1'">
                            <div class="flex">
                                <div class="select-wrap">
        ...
        <modal-default :class="{ 'show': modal.cancelShow }" :title="'취소신청'" :bodyClass="'p-2 type-cancel'" @modalClose="modal.cancelShow=false" class="modal-product">
            <form @submit.prevent="cancelProcess">
                ...
                <template v-for="(data,index) in cancelProp">
                    <PopupItem v-bind="data" :type="'cancel'"
                               :order-code="popup.cancelInfo.result.claimApply.orderCode"
...
export default {
    ...
    components: {PeriodSearch, Pagination, PopupItem, ModalDefault, OrderItem, AddressLayer},
    ...
    methods: {
        paging(param){
            ...
            this.$salesonApi().order.getOrderList(searchParam)
            ...
            $s.confirm("해당 상품을 구매확정 하시겠습니까?", function(){
                ...
                .then(function(response) {
                    if (response.data.status === 200) {
                        const order = self.order.result.content.filter(o => param.orderCode == o.orderCode)[0];
                        const reviewItem = order.items.filter(i=> i.itemSequence == param.itemSequence)[0];
                        self.popup.reviewInfo.result = reviewItem;
                        self.popup.reviewInfo.checkOrderCode = order.orderCode;
        ...
        // 후기작성
        reviewPopup(param){
            const order = this.order.result.content.filter(o => param.orderCode == o.orderCode)[0];
            const reviewItem = order.items.filter(i=> i.itemSequence == param.itemSequence)[0];
            this.popup.reviewInfo.result = reviewItem;
            this.popup.reviewInfo.checkOrderCode = order.orderCode;
        ...
        updateApplyQuantity(type, quantityObj, itemSequence){
            if (type == 'exchange'){
                this.popup.exchangeInfo.param.applyQuantity = quantityObj.quantity;
            }
            if (type == 'return'){
                this.popup.returnInfo.param.applyQuantity = quantityObj.quantity;
            }
            if (type == 'cancel'){
                this.popup.cancelInfo.param.claimQuantity.push(quantityObj);
            }
        },
        ...
        cancelProcess(){
            ...
            if (this.validateParams(param, 'cancel')){
        ...
        cancelRefundChange(obj){
            const order = this.order.result.content.filter(o => o.orderCode == obj.orderCode)[0];
        ...
        reviewParamDataConverter(){
            const order = this.order.result.content.filter(o => this.popup.reviewInfo.checkOrderCode == o.orderCode)[0];
        ...
        search(param) {
            param.searchStartDate = this.searchDateFormat(param.searchStartDate);
            param.searchEndDate = this.searchDateFormat(param.searchEndDate);
            this.param = param;
        },

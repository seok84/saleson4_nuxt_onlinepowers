<template>
    <div class="user-page sns-auth-page step2">
        ...
        <ValidationObserver ref="observer" v-slot="{ validate }">
            <form action="" class="sns-auth-form"  @submit.prevent="handleNext(validate)">
                <div class="sns-auth-input">
                    ...
                    <ValidationProvider tag="div" rules="required" v-slot="{errors}">
                        <div class="form-line" :class="{ 'invalid': errors.length }">
                            <input type="text" class="form-basic" v-model="authParam.userName" placeholder="이름을 입력하세요" />
                    ...
                    <ValidationProvider tag="div" rules="required|numeric|max:11" v-slot="{errors}">
                        <div class="form-line" :class="{ 'invalid': errors.length }">
                            <div class="flex">
                                <input type="text" class="form-basic" v-model="authParam.phoneNumber" placeholder="'-'제외 번호 입력" />
                                <button type="button" class="btn btn-black"
                                        v-if="authRequest" @click="sendAuthNumber()">인증요청</button><!-- 인증요청 -> 재전송 -->
                                <button type="button" class="btn btn-black" v-else @click="sendAuthNumber()">재전송</button><!-- 인증요청 -> 재전송 -->
                            </div>
                            <span class="feedback">{{ errors[0] }}</span>
                        </div>
                    </ValidationProvider>
                    ...
                    <ValidationProvider tag="div" rules="required" v-slot="{errors}">
                        <div class="form-line" :class="{ 'invalid': errors.length }">
                            <div class="flex">
                                <span class="time-limit" v-show="timerRunning">{{timeLimit}}</span>
                                <input type="text" id="auth-number" class="form-basic" v-model="authParam.authNumber" placeholder="인증번호를 입력하세요" />
                                <button type="button" class="btn btn-black" @click="checkAuthNumber()" v-if="!auth">인증하기</button>
                                <button type="button" class="btn btn-black" v-else>인증성공</button>
                            </div>
                            <span class="feedback">{{ errors[0] }}</span>
                        </div>
                    </ValidationProvider>
...
export default {
    ...
    async asyncData({$seo}) {
        try {
            ...
            console.log("[sms-certify.vue] asyncData error: ", error);
            return {}
        }
    },
    ...
    mounted() {
        if(!$s.api.getCookie("snsUser")) {
            $s.alert("인증이 완료되지 않았습니다.", () => {
                $s.redirect('/user/certify-join');
            })
        }

        sessionStorage.setItem("snsUser", decodeURIComponent($s.api.getCookie("snsUser")));
        $s.api.setCookie("snsUser", "",{'expires' : -1})
        ...
        async handleNext(validate) {
            ...
            if(isValid) {
                if (vm.auth) {
                    ...
                    sessionStorage.setItem("userName", vm.authParam.userName);
                    sessionStorage.setItem("phoneNumber", vm.authParam.phoneNumber);
                    $s.redirect('/user/sns-join');
